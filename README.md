# DNS-tracer
This script helps analysts to trace an internal endpoint that is querying malicious domains. It is not uncommon for analysts to have an alert which states the Source IP address of the DNS-server as the culprit, while actually the culprit is an endpoint behind the DNS-server. This PowerShell script uses Microsoft Event Trace Log (ETL) files to trace the true origin.
